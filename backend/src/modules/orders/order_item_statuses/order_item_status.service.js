const repo = require('./order_item_status.repository');

const pick  = (d) => { const o = {}; for (const k of ['code','name_fr','name_ar','color']) if (d[k] !== undefined) o[k] = d[k]; return o; };
const strip = (d) => ({ ...d, items_count: d._count?.order_items ?? 0, _count: undefined });
...
class OrderItemStatusService {
  ...
  async getById(id) {
    ...
    if (!item) throw { statusCode: 404, message: 'Statut ligne commande introuvable' };
    return strip(item);
  }
  async create(data) {
    const p = pick(data);
  ...
  async update(id, data) {
    if (!await repo.findById(id)) throw { statusCode: 404, message: 'Statut ligne commande introuvable' };
    const p = {};
  ...
  async delete(id) {
    ...
    if (used > 0) throw { statusCode: 409, message: `Ce statut est utilisé par ${used} ligne(s) — suppression impossible` };
    await repo.remove(id);
  }
  async seed() { return repo.seed(); }
}

module.exports = new OrderItemStatusService();
