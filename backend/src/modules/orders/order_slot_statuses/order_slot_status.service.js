...
class OrderSlotStatusService {
  ...
  async getById(id) {
    ...
    if (!item) throw { statusCode: 404, message: 'Statut créneau introuvable' };
    return item;
  }
  async create(data) {
    const p = pick(data);
  ...
  async update(id, data) {
    if (!await repo.findById(id)) throw { statusCode: 404, message: 'Statut créneau introuvable' };
    const p = {};
...
module.exports = new OrderSlotStatusService();
