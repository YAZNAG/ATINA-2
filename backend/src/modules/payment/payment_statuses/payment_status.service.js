const repo = require('./payment_status.repository');
...
class PaymentStatusService {
  ...
  async getById(id) {
    ...
    if (!item) throw { statusCode: 404, message: 'Statut paiement introuvable' };
    return strip(item);
  }
  async create(data) {
    const p = pick(data);
  ...
  async update(id, data) {
    if (!await repo.findById(id)) throw { statusCode: 404, message: 'Statut paiement introuvable' };
    const p = {};
  ...
  async delete(id) {
    ...
    if (used > 0) throw { statusCode: 409, message: `Ce statut est utilisé par ${used} paiement(s) — suppression impossible` };
