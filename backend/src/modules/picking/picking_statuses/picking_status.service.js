const repo = require('./picking_status.repository');
const PROTECTED = ['open','in_progress','completed','cancelled'];

const pick = (d) => { const o={}; for (const k of ['code','name_fr','name_ar']) if (d[k]!==undefined) o[k]=d[k]; return o; };
const strip = (d) => ({ ...d, sessions_count: d._count?.sessions ?? 0, _count: undefined });

class PickingStatusService {
  async getAll(params) {
  ...
  async getById(id) {
    ...
    if (!s) throw { statusCode:404, message:'Statut picking introuvable' };
    return strip(s);
  }
  async create(data) {
    const p = pick(data);
  ...
  async update(id, data) {
    ...
    if (!existing) throw { statusCode:404, message:'Statut picking introuvable' };
    const p = {};
  ...
  async delete(id) {
    const existing = await repo.findById(id);
    if (!existing) throw { statusCode:404, message:'Statut picking introuvable' };
    if (PROTECTED.includes(existing.code)) throw { statusCode:409, message:`Le statut «${existing.code}» est protégé et ne peut pas être supprimé` };
    const used = await repo.countUsage(id);
    if (used > 0) throw { statusCode:409, message:`Ce statut est utilisé par ${used} session(s) — suppression impossible` };
    await repo.remove(id);
  }
  async seed() { return repo.seed(); }
}
module.exports = new PickingStatusService();
