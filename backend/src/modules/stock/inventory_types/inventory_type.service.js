...
const ALLOWED_SCOPES = ['GLOBAL', 'NODE', 'LOCATION', 'ZONE', 'CATEGORY', 'PARTIAL'];
const FIELDS = ['code', 'name_fr', 'name_ar', 'scope', 'color', 'description_fr', 'is_active', 'sort_order'];
...
class InventoryTypeService {
  ...
  async create(data) {
    ...
    if (!payload.scope)   throw { statusCode: 400, message: 'Périmètre requis' };
    if (!ALLOWED_SCOPES.includes(payload.scope))
      throw { statusCode: 400, message: `Périmètre invalide — valeurs : ${ALLOWED_SCOPES.join(', ')}` };
