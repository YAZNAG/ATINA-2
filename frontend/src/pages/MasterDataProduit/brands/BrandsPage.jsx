...
export default function BrandsPage() {
  ...
  const fetchBrands = useCallback(async () => {
    ...
    try {
      ...
      showToast('error', err?.response?.data?.message || 'Erreur lors du chargement des marques');
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const t = setTimeout(fetchBrands, search ? 350 : 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        showToast('success', 'Marque créée');
      }
      setDrawerOpen(false);
      fetchBrands();
    } catch (err) {
  ...
  // ——— Activer / désactiver (indépendant du drawer) ———
    ...
    <div className="min-h-screen bg-neutral-50 p-6">
          ...
          <tbody className="divide-y divide-neutral-100">
            {loading ? (
              ...
              brands.map((b) => {
                ...
                return (
                  <tr key={b.id} className={`transition hover:bg-neutral-50 ${isDeleted ? 'opacity-60' : ''}`}>
                    <td className="px-4 py-3">
                      {b.logo ? (
                        <img src={b.logo} alt={b.name_fr} className="h-10 w-10 rounded-md object-cover" />
                      ) : (
      ...
      {pagination.pages > 1 && (
        <div className="mt-4 flex items-center justify-between text-sm text-neutral-500">
          <span>
            {pagination.total} marque{pagination.total > 1 ? 's' : ''} — page {pagination.page}/{pagination.pages}
          </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-neutral-500">
              ...
              "Supprimé". Cette action est réversible uniquement depuis la base de données.
            </p>
