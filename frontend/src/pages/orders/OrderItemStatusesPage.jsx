...
import { getOrderItemStatuses, createOrderItemStatus, updateOrderItemStatus, deleteOrderItemStatus, seedOrderItemStatuses } from '../../api/orders.api';
...
const PRESET_COLORS = [
  ...
  { label: 'Remplacé',     color: '#3b82f6' },
  { label: 'Rupture',      color: '#f97316' },
...
function hexColor(c) {
  ...
  const map = { green:'#10b981', red:'#ef4444', blue:'#3b82f6', orange:'#f97316',
    purple:'#8b5cf6', cyan:'#06b6d4', yellow:'#eab308', gray:'#64748b', indigo:'#6366f1' };
  return map[c] ?? '#64748b';
}
...
const inp = 'w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-sky-500 placeholder-gray-300';
...
const EMPTY = { code: '', name_fr: '', name_ar: '', color: '#10b981' };
...
function Drawer({ editItem, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await updateOrderItemStatus(editItem.id, { name_fr: form.name_fr, name_ar: form.name_ar, color: form.color });
      else        await createOrderItemStatus(form);
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-40 flex flex-col bg-white shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-sky-700 to-sky-500">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center">
              <Icon d={SVG.item} className="w-6 h-6 text-white" />
            </div>
            <div>
              <p className="text-white/60 text-[11px] font-semibold uppercase tracking-widest">{isEdit ? 'Modifier' : 'Nouveau'}</p>
              <h2 className="text-white font-bold text-xl">Statut ligne commande</h2>
            </div>
          </div>
        ...
        <form id="ois-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          <Fld label="Code" req>
            {isEdit ? (
          ...
          <Fld label="Nom (Français)" req>
            <input name="name_fr" className={inp} value={form.name_fr} onChange={hc} placeholder="Active" />
          </Fld>
          <Fld label="Nom (Arabe)" req>
            <input name="name_ar" className={inp} value={form.name_ar} onChange={hc} dir="rtl" placeholder="نشط" />
          </Fld>
        </form>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          <button type="submit" form="ois-form" disabled={saving} className="flex-1 py-2.5 text-sm font-semibold text-white bg-sky-600 hover:bg-sky-700 rounded-xl disabled:opacity-50">
...
export default function OrderItemStatusesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getOrderItemStatuses({ page, limit: PAGE_SIZE, ...(search && { search }) });
  ...
  const handleDelete = async () => {
    ...
    try { await deleteOrderItemStatus(deleting.id); toast.success('Statut supprimé'); setDeleting(null); load(); }
    catch (err) { toast.error(getErrorMessage(err)); }
  ...
  const handleSeed = async () => {
    setSeeding(true);
    try { await seedOrderItemStatuses(); toast.success('4 statuts seedés'); load(); }
    catch (err) { toast.error(getErrorMessage(err)); }
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <div className="flex items-center gap-2 text-xs text-gray-400 mb-1">
                ...
                <span className="text-sky-600 font-medium">Statuts ligne commande</span>
              </div>
              <h1 className="text-2xl font-bold text-gray-900">Statuts ligne commande</h1>
              <p className="text-sm text-gray-400 mt-0.5">Statut individuel de chaque article — active, cancelled, substituted, out_of_stock</p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button onClick={() => setDrawer(false)} className="flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setPage(1); setSearch(searchInput); }} className="flex items-center gap-2 max-w-sm">
            <div className="relative flex-1">
              ...
              <input type="search" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Rechercher…"
                className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 bg-white" />
            </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-20 h-20 rounded-2xl bg-sky-50 border border-sky-100 flex items-center justify-center">
              <Icon d={SVG.item} className="w-10 h-10 text-sky-200" />
            </div>
            <div className="text-center">
              <p className="text-gray-600 font-semibold">Aucun statut ligne commande</p>
            ...
            {!search && (
              <button onClick={handleSeed} disabled={seeding} className="px-5 py-2.5 border border-gray-200 text-gray-600 text-sm font-semibold rounded-xl hover:bg-gray-50 disabled:opacity-50">
                <Icon d={SVG.seed} className="w-4 h-4 inline mr-1.5" />Seed (4 statuts)
              </button>
            )}
          </div>
        ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="px-5 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide">Lignes</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${count > 0 ? 'bg-sky-50 text-sky-700' : 'bg-gray-100 text-gray-400'}`}>{count}</span>
                        </td>
                        <td className="px-5 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={() => setDeleting(item)} disabled={count > 0}
                              title={count > 0 ? `Utilisé par ${count} ligne(s)` : 'Supprimer'}
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
                    <button key={p} onClick={() => setPage(p)} className={`px-3 py-1.5 text-xs rounded-lg border ${page === p ? 'bg-sky-600 text-white border-sky-600' : 'border-gray-200 hover:bg-gray-50'}`}>{p}</button>
                  ))}
