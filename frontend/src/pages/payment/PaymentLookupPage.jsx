...
import { slugToSql, isAllowedPaymentSlug, PAYMENT_PARAM_ITEMS } from './paymentParamConfig';

const PAYMENT_HOME = '/payment/payment-statuses';
...
export default function PaymentLookupPage() {
  ...
  const sql = slug && isAllowedPaymentSlug(slug) ? slugToSql(slug) : null;
  const navLabel = PAYMENT_PARAM_ITEMS.find((x) => x.slug === slug)?.label ?? row?.labelFr ?? 'Réglage';
  ...
  if (!slug || !isAllowedPaymentSlug(slug)) {
  ...
  if (!row) {
    return (
      <div className="min-h-screen bg-gray-50 px-6 py-10">
        ...
        <Link to={PAYMENT_HOME} className="text-sm text-red-600 font-medium mt-4 inline-block">
          ← Retour paramétrage paiement
        </Link>
      </div>
    );
  }
  ...
  if (row.genericCrud === false) {
    return (
      <div className="min-h-screen bg-gray-50 px-6 py-10 max-w-3xl">
        ...
        <Link to={PAYMENT_HOME} className="text-sm text-red-600 font-medium mt-6 inline-block">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="px-6 pt-5 pb-4">
          <nav className="text-xs text-gray-400 mb-2 flex flex-wrap items-center gap-1.5">
            ...
            <span className="text-red-600 font-semibold">Paramétrage paiement</span>
            <span className="text-gray-300">/</span>
            <span className="text-gray-600">{navLabel}</span>
          </nav>
          <h1 className="text-2xl font-bold text-gray-900">{row.labelFr}</h1>
          <p className="text-sm text-gray-400 mt-0.5">
            Statuts et moyens de paiement — aligné sur le paramétrage stock et livraison.
          </p>
