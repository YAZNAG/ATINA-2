...
import { getPaymentStatuses, createPaymentStatus, updatePaymentStatus, deletePaymentStatus, seedPaymentStatuses } from '../../api/payment.api';
...
const STATUS_CONFIG = {
  pending:   { label: 'En attente',  badge: 'bg-orange-100 text-orange-700',  dot: 'bg-orange-400',  ring: 'ring-orange-200' },
  collected: { label: 'Collecté',    badge: 'bg-emerald-100 text-emerald-700', dot: 'bg-emerald-500', ring: 'ring-emerald-200' },
  failed:    { label: 'Échec',       badge: 'bg-red-100 text-red-700',         dot: 'bg-red-500',     ring: 'ring-red-200' },
  refunded:  { label: 'Remboursé',   badge: 'bg-blue-100 text-blue-700',       dot: 'bg-blue-500',    ring: 'ring-blue-200' },
};

function StatusBadge({ code }) {
  const cfg = STATUS_CONFIG[code] ?? { badge: 'bg-gray-100 text-gray-600', dot: 'bg-gray-400' };
  return (
...
const inp = 'w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-gray-300';
...
function Drawer({ editItem, onClose, onSaved }) {
  ...
  useEffect(() => {
    setForm(editItem ? { code: editItem.code ?? '', name_fr: editItem.name_fr ?? '', name_ar: editItem.name_ar ?? '' } : { ...EMPTY });
  }, [editItem]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await updatePaymentStatus(editItem.id, { name_fr: form.name_fr, name_ar: form.name_ar });
      else        await createPaymentStatus(form);
  ...
  const cfg = STATUS_CONFIG[form.code] ?? {};
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-40 flex flex-col bg-white shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-indigo-700 to-indigo-500">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center">
              <Icon d={SVG.pay} className="w-6 h-6 text-white" />
            </div>
            <div>
              <p className="text-white/60 text-[11px] font-semibold uppercase tracking-widest">{isEdit ? 'Modifier' : 'Nouveau'}</p>
              <h2 className="text-white font-bold text-xl">Statut paiement</h2>
            </div>
          </div>
            ...
            {isEdit ? (
              ...
              <>
                <input name="code" className={`${inp} font-mono`} value={form.code}
                  ...
                  placeholder="pending" />
                <p className="text-[11px] text-gray-400 mt-1">Minuscules, tirets bas — ex: pending, collected, failed</p>
              </>
            )}
          </Fld>
          {form.code && STATUS_CONFIG[form.code] && (
            <div className={`px-4 py-2.5 rounded-xl border ring-1 ${cfg.ring} ${cfg.badge?.replace(/text-\S+/, '')} flex items-center gap-2`}>
              <span className={`w-2 h-2 rounded-full ${cfg.dot}`} />
              <span className={`text-sm font-semibold ${cfg.badge?.split(' ').find(c => c.startsWith('text-'))}`}>{form.name_fr || cfg.label}</span>
            </div>
          )}
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          <button type="submit" form="ps-form" disabled={saving} className="flex-1 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl disabled:opacity-50">
...
export default function PaymentStatusesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getPaymentStatuses({ page, limit: PAGE_SIZE, ...(search && { search }) });
  ...
  const handleDelete = async () => {
    ...
    try { await deletePaymentStatus(deleting.id); toast.success('Statut supprimé'); setDeleting(null); load(); }
    catch (err) { toast.error(getErrorMessage(err)); }
  ...
  const handleSeed = async () => {
    setSeeding(true);
    try { await seedPaymentStatuses(); toast.success('4 statuts seedés'); load(); }
    catch (err) { toast.error(getErrorMessage(err)); }
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <div className="flex items-center gap-2 text-xs text-gray-400 mb-1">
                ...
                <span className="text-indigo-600 font-medium">Statuts paiement</span>
              </div>
              <h1 className="text-2xl font-bold text-gray-900">Statuts paiement</h1>
              <p className="text-sm text-gray-400 mt-0.5">Référentiel des états de paiement — commandes, remboursements, COD</p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button onClick={() => setDrawer(false)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setPage(1); setSearch(searchInput); }} className="flex items-center gap-2 max-w-sm">
            <div className="relative flex-1">
              ...
              <input type="search" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Rechercher…"
                className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white" />
            </div>
      ...
      <div className="px-6 pt-5">
        <div className="flex flex-wrap gap-3">
          ...
          {Object.entries(STATUS_CONFIG).map(([code, cfg]) => (
            <div key={code} className={`rounded-xl border px-4 py-3 min-w-[100px] ${cfg.badge?.replace(/text-\S+/, '')} ring-1 ${cfg.ring}`}>
              <p className={`text-xs font-semibold uppercase tracking-wide opacity-70 ${cfg.badge?.split(' ').find(c => c.startsWith('text-'))}`}>{code}</p>
              <p className={`text-2xl font-bold mt-0.5 ${cfg.badge?.split(' ').find(c => c.startsWith('text-'))}`}>
                {items.filter(i => i.code === code).length}
              </p>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-20 h-20 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center">
              <Icon d={SVG.pay} className="w-10 h-10 text-indigo-200" />
            </div>
            <div className="text-center">
              <p className="text-gray-600 font-semibold">Aucun statut de paiement</p>
            ...
            {!search && (
              <button onClick={handleSeed} disabled={seeding} className="px-5 py-2.5 border border-gray-200 text-gray-600 text-sm font-semibold rounded-xl hover:bg-gray-50 disabled:opacity-50">
                <Icon d={SVG.seed} className="w-4 h-4 inline mr-1.5" />Seed (pending, collected, failed, refunded)
              </button>
            )}
          </div>
        ) : (
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${count > 0 ? 'bg-indigo-50 text-indigo-700' : 'bg-gray-100 text-gray-400'}`}>{count}</span>
                        </td>
                        <td className="px-5 py-3.5 text-right">
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
                    <button key={p} onClick={() => setPage(p)} className={`px-3 py-1.5 text-xs rounded-lg border ${page === p ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-200 hover:bg-gray-50'}`}>{p}</button>
                  ))}
