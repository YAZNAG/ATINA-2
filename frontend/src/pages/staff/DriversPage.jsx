...
const VEHICLE_EMOJI = { moto:'🏍', camion:'🚛', voiture:'🚗', vélo:'🚴', van:'🚐', scooter:'🛵' };
function vEmoji(type) {
  if (!type) return '🚚';
  const k = Object.keys(VEHICLE_EMOJI).find(k => type.toLowerCase().includes(k));
  return k ? VEHICLE_EMOJI[k] : '🚚';
}
...
function Drawer({ editItem, nodes, onClose, onSaved }) {
  ...
  useEffect(() => { setForm(editItem ? { node_id:editItem.node_id??'', phone_country:editItem.phone_country??'+212', phone_number:editItem.phone_number??'', name:editItem.name??'', password:'', vehicle_type:editItem.vehicle_type??'', vehicle_plate:editItem.vehicle_plate??'' } : { ...EMPTY }); }, [editItem]);
      ...
      <div className="fixed inset-y-0 right-0 z-40 flex flex-col bg-white shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-emerald-700 to-emerald-500">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-white/60 text-[11px] font-semibold uppercase tracking-widest">{isEdit?'Modifier':'Nouveau'}</p><h2 className="text-white font-bold text-xl">Livreur</h2></div>
          </div>
        ...
        <form id="drv-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          <Fld label="Node" req>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Fld label="Type véhicule">
              <input name="vehicle_type" className={inp} value={form.vehicle_type} onChange={hc} placeholder="Moto, Camion…" list="vtypes" />
              <datalist id="vtypes">{['Moto','Scooter','Voiture','Van','Camion','Vélo'].map(v => <option key={v} value={v} />)}</datalist>
            </Fld>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          <button type="submit" form="drv-form" disabled={saving} className="flex-1 py-2.5 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl disabled:opacity-50">{saving?'Enregistrement…':isEdit?'Mettre à jour':'Créer'}</button>
        </div>
      </div>
    </>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={async () => { setSaving(true); try { await resetDriverPassword(item.id, pwd); toast.success('MDP réinitialisé'); onClose(); } catch(e){toast.error(getErrorMessage(e));} finally{setSaving(false);} }}
            disabled={saving || pwd.length < 6} className="flex-1 py-2.5 bg-emerald-600 text-white text-sm font-semibold rounded-xl disabled:opacity-50">{saving?'…':'Réinitialiser'}</button>
        </div>
      </div>
    </div>
  );
...
export default function DriversPage() {
  ...
  const [filters, setFilters] = useState({ node_id:'', is_active:'', vehicle_type:'', search:'' });
  ...
  const loadStats = useCallback(async () => {
    try {
      const [a, b, c] = await Promise.all([
        getDrivers({ limit:1 }),
        getDrivers({ limit:1, is_active:'true' }),
        getDrivers({ limit:1, is_active:'false' }),
      ]);
      setStats({ total: a.data?.pagination?.total??0, active: b.data?.pagination?.total??0, inactive: c.data?.pagination?.total??0 });
    } catch { /* ignore */ }
  }, []);
  ...
  const act = async (key, fn, msg) => {
    ...
    try { await fn(); toast.success(msg); load(); loadStats(); }
    catch (err) { toast.error(getErrorMessage(err)); }
  ...
  const handleDelete = async () => {
    ...
    try { await deleteDriver(deleting.id); toast.success('Livreur supprimé'); setDeleting(null); load(); loadStats(); }
    catch (err) { toast.error(getErrorMessage(err)); }
    ...
    <div className="min-h-screen bg-gray-50">
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            ...
            <h3 className="text-lg font-bold text-gray-900 text-center mb-1">Supprimer {deleting.name} ?</h3>
            <p className="text-sm text-gray-500 text-center mb-5">Suppression logique — compte désactivé.</p>
            <div className="flex gap-3">
              ...
              <button onClick={handleDelete} disabled={deleteLoading} className="flex-1 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl disabled:opacity-50">{deleteLoading?'…':'Supprimer'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-10">
        <div className="px-6 pt-5 pb-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select value={filters.is_active} onChange={e => pf('is_active', e.target.value)} className="text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500">
              <option value="">Tous statuts</option><option value="true">Actifs</option><option value="false">Inactifs</option>
            </select>
            <select value={filters.vehicle_type} onChange={e => pf('vehicle_type', e.target.value)} className="text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500">
              <option value="">Tous véhicules</option>
              {['Moto','Scooter','Voiture','Van','Camion','Vélo'].map(v => <option key={v} value={v}>{vEmoji(v)} {v}</option>)}
            </select>
            <span className="text-sm text-gray-400 ml-auto">{total} livreur{total!==1?'s':''}</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        <div className="flex flex-wrap gap-3">
          {[
            { l:'Total livreurs', v:stats.total,    c:'bg-gray-50 border-gray-200 text-gray-700' },
            { l:'Actifs',         v:stats.active,   c:'bg-emerald-50 border-emerald-200 text-emerald-700' },
            { l:'Inactifs',       v:stats.inactive, c:'bg-gray-50 border-gray-200 text-gray-500' },
          ].map(s => (
            <div key={s.l} className={`rounded-xl border px-4 py-3 ${s.c}`}>
              <p className="text-xs font-semibold opacity-70 uppercase tracking-wide">{s.l}</p>
              <p className="text-2xl font-bold mt-0.5">{s.v}</p>
            </div>
          ))}
        </div>
        ...
        {/* Table */}
        {loading ? (
          <div className="flex justify-center py-24"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-600" /></div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-20 h-20 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center justify-center text-4xl">🚚</div>
            <p className="text-gray-600 font-semibold">Aucun livreur</p>
            <button onClick={() => setDrawer(false)} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl">
              <Icon d={SVG.plus} className="w-4 h-4" />Créer un livreur
            </button>
          </div>
        ) : (
          <>
                  ...
                  {items.map(item => (
                    <tr key={item.id} onClick={() => navigate(`/staff/drivers/${item.id}`)} className="hover:bg-emerald-50/50 transition-colors group cursor-pointer">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-xl flex-shrink-0 border ${item.is_active?'bg-emerald-50 border-emerald-200':'bg-gray-50 border-gray-200'}`}>
                            {vEmoji(item.vehicle_type)}
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1.5">
                          <Icon d={SVG.node} className="w-3 h-3 text-gray-400" />
                      ...
                      <td className="px-4 py-3.5 text-center">
                        {item.is_active
                          ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />Actif</span>
                          : <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-50 text-gray-500 border border-gray-200"><span className="w-1.5 h-1.5 rounded-full bg-gray-400" />Inactif</span>}
                      </td>
                      <td className="px-4 py-3.5 text-right text-xs text-gray-400">{formatDate(item.created_at)}</td>
                      <td className="px-4 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                          <button onClick={() => navigate(`/staff/drivers/${item.id}`)} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Voir détail"><Icon d={SVG.eye} className="w-4 h-4" /></button>
                          ...
                          {item.is_active
                            ? <button onClick={() => act(`d-${item.id}`, () => deactivateDriver(item.id), 'Désactivé')} disabled={acting[`d-${item.id}`]} className="p-1.5 text-gray-500 hover:bg-gray-50 rounded-lg disabled:opacity-40"><Icon d={SVG.lock} className="w-4 h-4" /></button>
                            : <button onClick={() => act(`a-${item.id}`, () => activateDriver(item.id), 'Activé')} disabled={acting[`a-${item.id}`]} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg disabled:opacity-40"><Icon d={SVG.unlock} className="w-4 h-4" /></button>}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between">
                <p className="text-xs text-gray-400">Page {page}/{totalPages} · {total} livreur{total!==1?'s':''}</p>
