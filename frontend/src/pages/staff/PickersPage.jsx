  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-40 flex flex-col bg-white shadow-2xl w-full max-w-md">
        ...
        <form id="picker-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          <Fld label="Node" req>
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          <button type="submit" form="picker-form" disabled={saving} className="flex-1 py-2.5 text-sm font-semibold text-white bg-violet-600 hover:bg-violet-700 rounded-xl disabled:opacity-50">
...
function ResetModal({ item, onClose }) {
  ...
  const handleReset = async () => {
    ...
    setSaving(true);
    try { await resetPickerPassword(item.id, pwd); toast.success('Mot de passe réinitialisé'); onClose(); }
    catch (err) { toast.error(getErrorMessage(err)); }
    finally { setSaving(false); }
  };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6">
        <h3 className="text-lg font-bold text-gray-900 mb-1">Réinitialiser le MDP</h3>
        <p className="text-sm text-gray-500 mb-4"><strong>{item.name}</strong> — {item.phone_country} {item.phone_number}</p>
        <input type="password" value={pwd} onChange={e => setPwd(e.target.value)} placeholder="Nouveau mot de passe (min 6 car.)"
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 text-gray-600 text-sm font-semibold rounded-xl hover:bg-gray-50">Annuler</button>
          <button onClick={handleReset} disabled={saving} className="flex-1 py-2.5 bg-violet-600 text-white text-sm font-semibold rounded-xl hover:bg-violet-700 disabled:opacity-50">
...
export default function PickersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await getPickers(params);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mb-5">Suppression logique — le compte sera désactivé.</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => act(`del-${deleting.id}`, () => deletePicker(deleting.id), 'Picker supprimé').then(() => setDeleting(null))}
                className="flex-1 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700">Supprimer</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="px-6 pt-5 pb-4">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <button onClick={() => setDrawer(false)} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm flex-shrink-0">
              <Icon d={SVG.plus} className="w-4 h-4" />Nouveau picker
            </button>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select value={filters.is_active} onChange={e => pf('is_active', e.target.value)}
              className="text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-violet-500">
      ...
      <div className="px-6 py-5">
        {loading ? <div className="flex justify-center py-24"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-violet-600" /></div> : (
          <>
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {item.is_active
                            ...
                            : <button onClick={() => act(`a-${item.id}`, () => activatePicker(item.id), 'Activé')} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Activer"><Icon d={SVG.unlock} className="w-4 h-4" /></button>}
                          <button onClick={() => setResetItem(item)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Reset MDP"><Icon d={SVG.key} className="w-4 h-4" /></button>
                          <button onClick={() => setDeleting(item)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Supprimer"><Icon d={SVG.trash} className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <p className="text-xs text-gray-400">Page {page}/{totalPages} · {total}</p>
