      ...
      <div className="flex-1 p-5">
        <div className="flex items-start gap-4 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.name_ar && (
              <p className="text-xs text-gray-400 mt-0.5 truncate" dir="rtl">
                {item.name_ar}
              </p>
            )}
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-2">
              ...
              {item.color && (
                <span
                  className="inline-flex items-center gap-2 px-2 py-0.5 rounded-full text-[10px] font-semibold border border-gray-200 bg-white"
                  title={item.color}
                >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
        ...
        <p className="text-sm text-gray-500 text-center mb-6">
          Le statut <span className="font-semibold text-gray-700">«{item?.name_fr}»</span> sera supprimé
          définitivement.
        </p>
...
function InventoryStatusDrawer({ editItem, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await updateInventoryStatus(editItem.id, form);
      else await createInventoryStatus(form);

      toast.success(isEdit ? 'Statut mis à jour' : 'Statut inventaire créé');
      onSaved();
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-40 flex flex-col bg-white shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-red-700 to-red-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-white font-bold text-xl">Statut inventaire</h2>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form id="inv-status-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <Fld label="Nom (Français)" req>
            <input name="name_fr" className={inp} value={form.name_fr} onChange={hc} placeholder="Disponible" />
          </Fld>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className={`text-sm font-semibold ${form.is_active ? 'text-red-800' : 'text-gray-600'}`}>Statut actif</p>
                <p className="text-xs text-gray-400">Disponible à l’utilisation dans le système</p>
              </div>
            </div>
...
export default function InventoryStatusesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getInventoryStatuses({ limit: 500 });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteInventoryStatus(deleting.id);
  ...
  const swap = async (item, direction) => {
    ...
    try {
      await Promise.all([
        updateInventoryStatus(item.id, { sort_order: neighbor.sort_order }),
        updateInventoryStatus(neighbor.id, { sort_order: item.sort_order }),
      ]);
      await load();
    } catch (err) {
      toast.error(getErrorMessage(err));
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="px-6 pt-5 pb-4">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Statuts inventaire</h1>
              <p className="text-sm text-gray-400 mt-0.5">Code, noms FR/AR, couleur et activation</p>
            </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            >
              ...
              <button type="submit" className="px-3 py-2 bg-gray-800 hover:bg-gray-700 text-white text-sm font-medium rounded-xl">
                OK
              </button>
              ...
              {search && (
                <button
                  ...
                  className="px-2 py-2 border border-gray-200 text-gray-500 text-sm rounded-xl hover:bg-gray-50"
                >
                  ×
                </button>
              )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <div className="text-center">
              <p className="text-gray-600 font-semibold">Aucun statut inventaire</p>
