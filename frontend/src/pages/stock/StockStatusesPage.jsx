...
import { getStockStatuses, createStockStatus, updateStockStatus, deleteStockStatus } from '../../api/stock.api';
...
const PRESET_COLORS = [
  ...
  { label: 'Réservé',     color: '#3b82f6' },
  { label: 'Bloqué',      color: '#ef4444' },
  { label: 'Endommagé',   color: '#f59e0b' },
  { label: 'Expiré',      color: '#64748b' },
  { label: 'Transit',     color: '#8b5cf6' },
  { label: 'Quarantaine', color: '#d97706' },
  { label: 'Rappel',      color: '#dc2626' },
];

// ── Status Card ───────────────────────────────────────────────────────────────

function StatusCard({ item, onEdit, onDelete, onMoveUp, onMoveDown, isFirst, isLast }) {
      ...
      <div className="flex-1 p-5">
        <div className="flex items-start gap-4 mb-3">
          ...
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm"
            ...
            <Icon d={SVG.tag} className="w-7 h-7 text-white" />
          </div>
...
const EMPTY = { code: '', name_fr: '', name_ar: '', color: '#10b981', is_sellable: false, is_active: true, sort_order: 0 };

function StatusDrawer({ editItem, onClose, onSaved }) {
  ...
  useEffect(() => {
    setForm(editItem ? {
      ...
      is_sellable: editItem.is_sellable ?? false,
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await updateStockStatus(editItem.id, form);
      else        await createStockStatus(form);
      toast.success(isEdit ? 'Statut mis à jour' : 'Statut de stock créé');
      onSaved();
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-40 flex flex-col bg-white shadow-2xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-red-700 to-red-500">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center">
              <Icon d={SVG.status} className="w-6 h-6 text-white" />
            </div>
            <div>
              <p className="text-white/60 text-[11px] font-semibold uppercase tracking-widest">
                {isEdit ? 'Modifier' : 'Nouveau'}
              </p>
              <h2 className="text-white font-bold text-xl">Statut de stock</h2>
            </div>
          </div>
        ...
        {/* Form */}
        <form id="status-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Fld label="Ordre d'affichage">
              <input name="sort_order" type="number" min="0" className={inp} value={form.sort_order}
                onChange={hc} placeholder="0" />
            </Fld>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className={`flex items-center justify-between p-4 rounded-xl border transition-colors ${
              form.is_sellable ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50 border-gray-200'
            }`}>
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                  form.is_sellable ? 'bg-emerald-100' : 'bg-gray-200'
                }`}>
                  <Icon d={SVG.cart} className={`w-5 h-5 ${form.is_sellable ? 'text-emerald-600' : 'text-gray-400'}`} />
                </div>
                <div>
                  <p className={`text-sm font-semibold ${form.is_sellable ? 'text-emerald-800' : 'text-gray-600'}`}>
                    Vendable (is_sellable)
                  </p>
                  <p className="text-xs text-gray-400">
                    Le stock avec ce statut peut être vendu / expédié
                  </p>
                </div>
              </div>
              <div onClick={() => toggle('is_sellable')}
                className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer flex-shrink-0 ${
                  form.is_sellable ? 'bg-emerald-500' : 'bg-gray-300'
                }`}>
                <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
                  form.is_sellable ? 'translate-x-5' : ''
                }`} />
              </div>
            </div>
            ...
            <div className={`flex items-center justify-between p-4 rounded-xl border transition-colors ${
              ...
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                  form.is_active ? 'bg-red-100' : 'bg-gray-200'
                }`}>
                  <Icon d={SVG.status} className={`w-5 h-5 ${form.is_active ? 'text-red-600' : 'text-gray-400'}`} />
                </div>
                <div>
                  <p className={`text-sm font-semibold ${form.is_active ? 'text-red-800' : 'text-gray-600'}`}>
                    Statut actif
                  </p>
                  <p className="text-xs text-gray-400">
                    Statut disponible à l'utilisation dans le système
                  </p>
                </div>
              </div>
          ...
          {/* Warning if not sellable */}
          {!form.is_sellable && form.name_fr && (
            <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-200">
              <Icon d={SVG.warn} className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
              <p className="text-xs text-amber-700">
                Le stock avec ce statut <strong>ne peut pas être vendu</strong> ni expédié.
              </p>
            </div>
          )}
        </form>
...
export default function StockStatusesPage() {
  ...
  const [filter, setFilter]         = useState('all');   // all | active | inactive | sellable
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getStockStatuses({ limit: 500 });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteStockStatus(deleting.id);
  ...
  // Sort order swap helpers
  const swap = async (item, direction) => {
    ...
    try {
      await Promise.all([
        updateStockStatus(item.id,     { sort_order: neighbor.sort_order }),
        updateStockStatus(neighbor.id, { sort_order: item.sort_order }),
      ]);
      await load();
    } catch (err) { toast.error(getErrorMessage(err)); }
  };
  ...
  const filtered = items.filter((item) => {
    ...
    if (filter === 'sellable' && !item.is_sellable) return false;
    if (q && !(item.name_fr ?? '').toLowerCase().includes(q) && !(item.code ?? '').toLowerCase().includes(q)) return false;
    return true;
  });

  const sortedFiltered = [...filtered].sort((a, b) => a.sort_order - b.sort_order || a.name_fr.localeCompare(b.name_fr));
  ...
  const stats = [
    ...
    { label: 'Vendables', value: items.filter((i) => i.is_sellable).length,  color: 'bg-blue-50 border-blue-100 text-blue-700' },
    { label: 'Bloqués',   value: items.filter((i) => !i.is_sellable).length, color: 'bg-slate-50 border-slate-100 text-slate-600' },
  ];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="px-6 pt-5 pb-4">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Statuts de stock</h1>
              <p className="text-sm text-gray-400 mt-0.5">
                Disponible, réservé, bloqué, endommagé, expiré…
              </p>
            </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs text-gray-400 ml-auto">
              {sortedFiltered.length} statut{sortedFiltered.length !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pt-4 pb-0">
        <div className="flex gap-3 flex-wrap">
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl border text-xs bg-emerald-50 border-emerald-200">
            <Icon d={SVG.cart} className="w-3.5 h-3.5 text-emerald-600" />
            <span className="font-semibold text-emerald-700">Vendable</span>
            <span className="text-gray-400">—</span>
            <span className="text-gray-500">Stock utilisable pour vente / expédition</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl border text-xs bg-gray-50 border-gray-200">
            <Icon d={SVG.lock} className="w-3.5 h-3.5 text-gray-500" />
            <span className="font-semibold text-gray-600">Non vendable</span>
            <span className="text-gray-400">—</span>
            <span className="text-gray-500">Stock bloqué, immobilisé ou en traitement</span>
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-20 h-20 rounded-2xl flex items-center justify-center bg-red-50 border border-red-100">
              <Icon d={SVG.status} className="w-10 h-10 text-red-200" />
            </div>
            <div className="text-center">
              <p className="text-gray-600 font-semibold">Aucun statut de stock</p>
              <p className="text-gray-400 text-sm mt-1">
                {search || filter !== 'all' ? 'Modifiez vos filtres.' : 'Cliquez sur « Nouveau statut » pour commencer.'}
              </p>
            </div>
            {!search && filter === 'all' && (
              <button onClick={() => setDrawer({ editItem: null })}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            <button onClick={() => setDrawer({ editItem: null })}
              ...
              <span className="text-sm font-semibold">Ajouter un statut</span>
            </button>
          </div>
        )}
      </div>
